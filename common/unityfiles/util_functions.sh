...
patch_script() {
  ...
  for i in "ROOT" "MAGISK" "LIBDIR" "SYSOVER" "MODID" "MOUNTEDROOT"; do
    sed -i "4i $i=$(eval echo \$$i)" $1
  done
  if $MAGISK; then
    sed -i -e "s|\$MODPATH|$MOUNTEDROOT|g" -e "s|\$MOUNTPATH|$(basename $MOUNTEDROOT)|g" -e "s|\$MODULEROOT|$(basename $MOUNTEDROOT)|g" -e "10i INFO=$MOUNTEDROOT/$MODID-files" $1
  else
    sed -i -e "s|\$MODPATH||g" -e "s|\$MOUNTPATH||g" -e "s|\$MODULEROOT||g" -e "10i INFO=$INFO" $1
  fi
}
